 ...
 *                      survive; docs older than --max-age-days still marked
 *                      "new" are queued for deletion.
 *
...
export function planWrites(results, existing = {}, { now = Date.now(), maxAgeDays = 14 } = {}) {
  const sets = [], deletes = [];
  const seen = new Set();
  for (const c of results) {
    const { id, doc } = toDoc(c, now);
    if (seen.has(id)) continue; seen.add(id);
    if (existing[id]) continue;                       // already there — keep the user's status
...
function main() {
  ...
  const { sets, deletes } = planWrites(results, existing, { maxAgeDays: args["max-age-days"] ? Number(args["max-age-days"]) : 14 });
  ...
  writeFileSync(join(outDir, "manifest.json"), JSON.stringify({ collection: "discoveries", sets: sets.length, deletes: deletes.length, skippedExisting: Object.keys(existing).length ? results.length - sets.length : 0, batches }, null, 2));
  const dupes = results.length - new Set(results.map((c) => toDoc(c).id)).size;
  console.log(`${sets.length} new discoveries to write, ${deletes.length} stale to delete, ${results.length - sets.length - dupes} already in Scout, ${dupes} duplicate(s) in the scan → ${batches.length} batch(es) in ${join(outDir, "manifest.json")}`);
}
